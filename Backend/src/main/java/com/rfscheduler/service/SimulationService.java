...
public class SimulationService {
    ...
    private void runSimulationLoop(SimulationEntity sim, String policyType) {
        ...
        for (long t = startStep + 1; t <= durationSteps; t++) {
            ...
            metricsEngine.recordStep(eventOpt, reward, 0L, action.nextBandId(), isHighPriority);
    ...
    /**
     * Get the MetricsSummary for a running or completed simulation (used by metrics endpoint).
     */
    public MetricsSummary getLiveMetrics(String simulationId) {
        // Return a basic metrics summary from scan/detection events in DB
        // For live metrics, we'd keep the MetricsEngine in memory; for now, return defaults
