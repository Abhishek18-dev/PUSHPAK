...
export const Scheduler: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Poll scheduler status and decision info every 3 seconds to exercise API contract §2
  ...
  const handleUpdatePolicy = async (e: React.FormEvent) => {
    e.preventDefault();
    const res = await api.scheduler.updateConfig(policy);
    if (res.success) {
      alert(`Scheduler policy updated to: ${policy}`);
      fetchSchedulerStatus();
    } else {
      alert(`Failed to update policy: ${res.error?.message}`);
    }
  };
  ...
  const handleStartScheduler = async () => {
    ...
    if (res.success) {
      ...
      alert(`Failed to start: ${res.error?.message}`);
    }
  };
  ...
  const handleStopScheduler = async () => {
    ...
    if (res.success) {
      ...
      alert(`Failed to stop: ${res.error?.message}`);
    }
  };
