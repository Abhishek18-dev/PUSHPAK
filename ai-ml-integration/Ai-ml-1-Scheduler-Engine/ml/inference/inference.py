...
class InferenceEngine:
    ...
    def _build(self, policy: str, num_bands: int, model_id: str | None) -> tuple[Agent, str]:
        ...
        # In live deployment, set a balanced exploration rate (eps=0.20) so the agent
        # discovers active emitters across spectrum bands and avoids getting locked on one band.
    ...
    def decide(
        ...
        # explore=True: enables epsilon-greedy exploration so the agent sweeps and discovers
        # all emitters across the RF spectrum rather than camping on a single band.
        action_index = agent.select_action(vector, explore=True)
